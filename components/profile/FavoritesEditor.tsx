...
export function FavoritesEditor({
  ...
  async function addMovie(movie: SearchResult) {
    if (items.length >= MAX_FAVORITES) return;
    if (items.some((i) => i.tmdbId === movie.id)) return;
  ...
  async function removeMovie(tmdbId: number) {
    const next = items.filter((i) => i.tmdbId !== tmdbId);
    setItems(next);
    await persist(next);
  }
  ...
  const isFull = items.length >= MAX_FAVORITES;
